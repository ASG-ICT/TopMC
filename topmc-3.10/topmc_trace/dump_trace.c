...
void test_topmc_trace(int fd)
{
	int command;
	int wr_cnt;

	command = TOPMC_END_AND_DUMP_TRACE;
	//command = CMD_IOREMAP_WRITE_TEST;
	wr_cnt = write(fd, &command, sizeof(int));

	printf("test_topmc_trace(): command:%d\n", command);	
	if(wr_cnt != sizeof(int))
		printf("Begin test_topmc_trace: write 0 failure!\n");
	else
		printf("Begin test_topmc_trace: write 0 success!\n");
