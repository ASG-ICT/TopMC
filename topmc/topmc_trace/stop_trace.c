...
void stop_topmc_trace(int fd)
{
	int command;
	int wr_cnt;

	command = TOPMC_SCHD_STOP_TRACE;
	wr_cnt = write(fd, &command, sizeof(int));
	
	if(wr_cnt != sizeof(int))
		printf("Begin stop_topmc_trace: write 0 failure!\n");
	else
		printf("Begin stop_topmc_trace: write 0 success!\n");
