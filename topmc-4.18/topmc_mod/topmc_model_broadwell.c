...
#include "topmc_msr.h"
#include "topmc_pmc_config_intel_broadwell.h"
...
//int topmc_enable_all_read_broadwell(char *page, char **start, off_t off, int count, int *eof, void *data)
int topmc_enable_all_read_broadwell(struct file *filp,char *buffer,size_t count,loff_t *offp ) 
{
	char * data;
...
//int topmc_enable_all_write_broadwell(struct file *file, const char __user * buffer, unsigned long count, void * data)
int topmc_enable_all_write_broadwell(struct file *filp, const char *buffer,size_t count,loff_t *offp ) 
{
	char enable;
	int cpu, counter;
	//      unsigned short cpu,counter;
...
//int topmc_enable_read_broadwell(char *page, char **start, off_t off, int count, int *eof, void *data)
int topmc_enable_read_broadwell(struct file *filp,char __user *buffer,size_t count,loff_t *offp ) 
{
	char * data;
	data=PDE_DATA(file_inode(filp));
	if(!(data)){
...
//	if (copy_to_user(buffer, tmpchar, 2* sizeof(char))) {
	if (raw_copy_to_user(buffer, tmpchar, 2* sizeof(char))) {
		  return - EFAULT;
	}
...
//int topmc_enable_write_broadwell(struct file *file, const char __user * buffer, unsigned long count, void * data)
int topmc_enable_write_broadwell(struct file *filp,const char *buffer,size_t count,loff_t *offp ) 
{
	char * data;
	data=PDE_DATA(file_inode(filp));
	if(!(data)){
...
*/
//int topmc_event_read_broadwell(char *page, char **start, off_t off, int count, int *eof, void *data)
int topmc_event_read_broadwell(struct file *filp,char *buffer,size_t count,loff_t *offp ) 
{
	char * data;
	data=PDE_DATA(file_inode(filp));
	if(!(data)){
...
//	printk("tmpstore = %s , len = %d\n",tmpstore,strlen(tmpstore));
        if (raw_copy_to_user(buffer, tmpstore, strlen(tmpstore) * sizeof(char))) {
                  return - EFAULT;
        }
...
//int topmc_event_write_broadwell(struct file *file, const char __user * buffer, unsigned long count, void * data)
int topmc_event_write_broadwell(struct file *filp,const char *buffer,size_t count,loff_t *offp ) 
{
	char * data;
	data=PDE_DATA(file_inode(filp));
	if(!(data)){
...
#ifdef TOPMC_PER_SECOND
u64 topmc_value_read_callable_impl_broadwell(unsigned short cpu, unsigned short counter)
{
	u64 val;
	u32 lo,hi;
	...
	if(counter>=TOPMC_MSR_XEON55XX_PerfCtrNum + TOPMC_U_MSR_PMON_PerfCtrNum + TOPMC_PCU_MSR_PMON_PerfCtrNum + TOPMC_CBox_MSR_PMON_PerfCtrNum){
		printk("+->topmc: topmc_value_read_callable_impl_broadwell(): counter err : %d!!!\n",counter);
...
//int topmc_value_read_broadwell(char *page, char **start, off_t off, int count, int *eof, void *data)
int topmc_value_read_broadwell(struct file *filp,char *buffer,size_t count,loff_t *offp ) 
{
	char * data;
	data=PDE_DATA(file_inode(filp));
	if(!(data)){
...
//	printk("now int read value func,cpu = %d , counter = %d\n",cpu ,counter);
	...
	if(counter>=TOPMC_MSR_XEON55XX_PerfCtrNum + TOPMC_U_MSR_PMON_PerfCtrNum + TOPMC_PCU_MSR_PMON_PerfCtrNum + TOPMC_CBox_MSR_PMON_PerfCtrNum){
		printk("+->topmc: topmc_value_read_broadwell(): counter err : %d!!!\n",counter);
...
//int topmc_value_write_broadwell(struct file *file, const char __user * buffer, unsigned long count, void * data)
int topmc_value_write_broadwell(struct file *filp,const char *buffer,size_t count,loff_t *offp ) 
{
	char * data;
	data=PDE_DATA(file_inode(filp));
	if(!(data)){
...
void topmc_msr_init_broadwell(void)
{
	int i,cpu, core;
	//int max_cores=6;
	u64 val;
...
void topmc_core_global_ctrl_init_broadwell(void)
{
	u64 mask;
	int cpu;
...
void topmc_uncore_global_ctrl_init_broadwell(void)
{
#if 0 	//Nehalem
	u64 mask;
	int cpu;
...
struct topmc_x86_model topmc_op_intel_broadwell = {
	...
	.topmc_msr_init = &topmc_msr_init_broadwell,
	.topmc_core_global_ctrl_init = &topmc_core_global_ctrl_init_broadwell,
	.topmc_uncore_global_ctrl_init = &topmc_uncore_global_ctrl_init_broadwell,
	.topmc_enable_all_read = &topmc_enable_all_read_broadwell,
	.topmc_enable_all_write = &topmc_enable_all_write_broadwell,
	.topmc_enable_read = &topmc_enable_read_broadwell,
	.topmc_enable_write = &topmc_enable_write_broadwell,
	.topmc_event_read = &topmc_event_read_broadwell,
	.topmc_event_write = &topmc_event_write_broadwell,
	#ifdef TOPMC_PER_SECOND
	.topmc_value_read_callable_impl = &topmc_value_read_callable_impl_broadwell,
	#endif
	.topmc_value_read = &topmc_value_read_broadwell,
	.topmc_value_write = &topmc_value_write_broadwell
};
