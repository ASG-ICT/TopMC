...
#define USE_TOPMC //performance monitoring counter like top : yuanbor@2009-2-24   modified by huangy@2010-3-24
#ifdef  USE_TOPMC
...
{                       
	...
	topmc_core_global_ctrl_init();
        ...
        entry   = create_proc_entry("enable_all",0666,root_dir);
        if(entry){
                entry->read_proc        = topmc_enable_all_read;
                entry->write_proc       = topmc_enable_all_write;
        }
        ...
        for_each_online_cpu(cpu){
                ...
                core_dir        = proc_mkdir(namebuf,root_dir);
                if(!core_dir){
                ...
                for(counter=0;counter<TOPMC_MSR_XEON55XX_PerfCtrNum + TOPMC_MSR_XEON55XX_UNC_PerfCtrNum;counter++){
                        ...
                        counter_dir        = proc_mkdir(namebuf,core_dir);
                        if(!counter_dir){
                                printk("mkprocdir %s  error\n",namebuf);
                                return;
                        }
                        ...
                        entry   = create_proc_entry("enable",0666,counter_dir);
                        if(entry){
                                entry->data                     = (void *)(unsigned long)mix_cpu_counter;
                                entry->read_proc        = topmc_enable_read;
                                entry->write_proc       = topmc_enable_write;
                        }

                        entry   = create_proc_entry("event",0666,counter_dir);
                        if(entry){
                                entry->data                     = (void *)(unsigned long)mix_cpu_counter;
                                entry->read_proc        = topmc_event_read;
                                entry->write_proc       = topmc_event_write;
                        }

                        entry   = create_proc_entry("value",0666,counter_dir);
                        if(entry){
                                entry->data                     = (void *)(unsigned long)mix_cpu_counter;
                                entry->read_proc        = topmc_value_read;
                                entry->write_proc       = topmc_value_write;
                        }
                }
        }
