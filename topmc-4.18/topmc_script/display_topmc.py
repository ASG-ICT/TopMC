...
def get_max_width(table, index):
    ...
    for row in table:
	if(index < len(row)):
	    if(maxlen < len(format_num(row[index]))):
		maxlen = len(format_num(row[index]))
	...
	for dir in full_dir_items:
		...
		if cpu==0:
			...
			if 'enable' in dir:
				...
				enable_flag[counter] = enable_flag0
#				print "enable_flag[counter] =",enable_flag[counter],"counter = ",counter
				if enable_all=='0' and enable_flag0=='1':
					enable_all = '1'
				f.close()
#	print "uncore_counter:",uncore_counter
#	print "counter:",counter 
	...
	for dir in full_dir_items:
		...
		if 'uncore_counter' in dir:
			uncore_counter = int(dir[dir.index('uncore_counter')+14:dir.index('uncore_counter')+15])
			counter = int(uncore_counter) + 4 ## 4 is the total number of incore counters
#		print "enable_flag[counter] = ",enable_flag[counter]
...
#					print "value_buffer[cpu][counter]=",value_buffer[cpu][counter],"cpu=",cpu,"cpunter=",counter
				else:
					buf = {}
...
#					print "event_buffer[cpu][counter]=",event_buffer[cpu][counter],"cpu=",cpu,"cpunter=",counter
				else:
					buf = {}
					buf[counter] = f.readline().strip()
